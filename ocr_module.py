...
class EnhancedThaiDocumentOCR:
    ...
    # ============================================================
    # 🔹 Extract ฟิลด์สำคัญจากเอกสารราชการ
    # ============================================================
    def extract_key_fields(self, text):
        fields = {}
        match = re.search(r'เลขที่[:\s]*([^\n]+)', text)
        if match: fields['เลขที่'] = match.group(1).strip()
        match = re.search(r'วันที่[:\s]*(\d+[^\n]+)', text)
        if match: fields['วันที่'] = match.group(1).strip()
        match = re.search(r'เรื่อง[:\s]*([^\n]+(?:\n(?!\s*เรียน)[^\n]+)*)', text)
        if match: fields['เรื่อง'] = match.group(1).strip()
        match = re.search(r'เรียน[:\s]*([^\n]+)', text)
        if match: fields['เรียน'] = match.group(1).strip()
        return fields
